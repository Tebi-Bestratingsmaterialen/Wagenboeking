...
export default async function handler(req, res) {
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: ['it@tebi.nl'],
      subject: `Nieuwe wagenboeking – ${naam} – ${datumFormatted}`,
      html: `
        <div style="font-family: -apple-system, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px 24px;">
          ...
          <h1 style="font-size: 1.3rem; font-weight: 700; color: #1a1a1a; margin: 0 0 6px;">Nieuwe boeking geplaatst</h1>
          <p style="color: #8a8a8a; margin: 0 0 24px; font-size: 0.9rem;">${naam} heeft zojuist een bedrijfswagen geboekt.</p>
          <div style="background: #f7f8fa; border: 1px solid #e8e8ec; border-radius: 10px; padding: 20px; margin-bottom: 28px;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr>
                <td style="padding: 8px 0; color: #8a8a8a; font-size: 0.85rem;">E-mail boeker</td>
                <td style="padding: 8px 0; font-weight: 600; color: #1a1a1a;">${email || '-'}</td>
              </tr>
            </table>
          </div>
          <p style="color: #8a8a8a; font-size: 0.82rem; margin: 0;">
            Deze mail komt tijdelijk bij IT binnen omdat het afzenderdomein nog niet geverifieerd is bij Resend.<br><br>
